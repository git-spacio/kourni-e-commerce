...
def create_final_dataframe_for_sheets(grouped_lists_with_volume):
    """
    Crea el DataFrame final en el formato deseado para ser actualizado en Google Sheets.
    """
    final_df = pd.DataFrame(columns=['Group', 'Volume'])

    for group in grouped_lists_with_volume:
        group_total_volume = group[0]
        keywords_with_volumes = group[1:]

        # Añadir la fila del grupo con su volumen total
        ...
        # Añadir las palabras clave con sus volúmenes
        for keyword, volume in keywords_with_volumes:
            final_df = pd.concat([final_df, pd.DataFrame({'Group': [keyword], 'Volume': [volume]})], ignore_index=True)
...
def main(thresholds, embedded_path, keywords_stats_file, id_sheets, page_name):
    ...
    # Crear listas agrupadas con volumen
    grouped_lists_with_volume = create_grouped_lists_with_volume(final_groupings, group_names)

    # Ordenar las listas agrupadas por el volumen total de mayor a menor
    grouped_lists_with_volume.sort(key=lambda x: x[0], reverse=True)

    # Crear el DataFrame final para actualizar en Google Sheets
